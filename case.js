		...
		switch (command) {		
			...
			case 'upsaluran': {
				...
				try {
					...
					let defaultCaption = "✨ Media ini dikirim melalui sistem otomatis SyadBotz! ✨"
					if (/image/.test(mime)) {
						...
						newReply(`Hmm... SyadBotz gak tau ini jenis media apa. Coba dicek lagi ya, kak! 🧐`)
					}
				} catch (error) {
					console.error(error)
			...
			case 'searchnumber': {
				...
				newReply('Tunggu sebentar ya kak, SyadBotz lagi cari nomornya... 🔍✨');
				...
				finalMessage += `\n${nobio}`;
				finalMessage += `\n${nowhatsapp}`;
				finalMessage += `\nKalau butuh bantuan lagi, panggil SyadBotz ya! 🤗`;
			...
			case 'tes':
			case 'test': {
				const caption = `Haii, Kak! ✨ SyadBotz udah aktif nih dan siap nemenin Kakak kapan aja! 🤗💕\nKalau ada yang mau Kakak mulai atau mau cek seberapa cepat respons SyadBotz, langsung aja klik tombol di bawah ya! 👇✨`;

				haruka.sendMessage(m.chat, {
					image: thumb,
					caption: caption,
					footer: `${botName} • SyadBotz siap sedia buat Kakak! 💬`,
					buttons: [
						{
							buttonId: `${prefix}menu`,
			...
			case 'tiktok': {
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				if (!text) return newReply(`⚠️ Hmm... kakak belum kasih link nih! 🫣 Coba ketik kayak gini ya: *${prefix + command} https://vt.tiktok.com/ZS8KdFQcQ/* biar SyadBotz bisa bantu! 🎥✨`);
				try {
					await reactionMessage('⏱️');
			...
			case 'ttsearch': {
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				if (!text) return newReply(`⚠️ Eits, kakak lupa kasih kata kunci! 😗 Coba ketik kayak gini ya: *${prefix + command} jj epep* biar SyadBotz bisa bantu cari yang kakak mau! 🔍💬`);
				try {
					await reactionMessage('⏱️');
			...
			case 'scsearch': {
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				if (!text) return newReply(`⚠️ Eits, kakak lupa kasih kata kunci! 😗 Coba ketik kayak gini ya: *${prefix + command} DJ mama muda* biar SyadBotz bisa bantu cari yang kakak mau! 🔍💬`);
				try {
			...
			case 'bukalapak': {
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				if (!text) return newReply(`⚠️ Uh-oh, kakak lupa kasih kata kunci nih! 🫣 Coba ketik kayak gini ya: *${prefix + command} iPhone 15 Case* biar SyadBotz bisa bantu cari produknya! 🛒✨`);
				try {
			...
			case 'playstore': {
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				if (!text) return newReply(`⚠️ Uh-oh, kakak lupa kasih kata kunci nih! 🫣 Coba ketik kayak gini ya: *${prefix + command} WhatsApp* biar SyadBotz bisa bantu cari aplikasinya! 📲✨`);
			...
			case 'quotesanime': {
				...
				try {
					...
					if (!quotes || quotes.length === 0) {
						return newReply(`⚠️ Wah, SyadBotz gak nemu quote anime nih, Kak! Coba lagi nanti ya 🥲`);
					}
			...
			case 'anime': {
				...
				await newReply('⏳ *Tunggu sebentar ya, SyadBotz lagi cari datanya...* 📚✨');
			...
			case 'simi': {
				...
				try {
					...
					if (!response) return m.reply(`⚠️ SyadBotz gak dapet jawaban dari SimSimi, Kak! 🥲`);
			...
			case 'wallpaper': {
				...
				try {
					...
					if (wallpapers.length === 0) return newReply(`⚠️ SyadBotz gak nemu wallpaper dengan kata kunci "${query}", Kak! 🥲`);
			...
			case 'wikimedia': {
				...
				try {
					...
					if (results.length === 0) return newReply(`⚠️ SyadBotz gak nemu gambar di Wikimedia dengan kata kunci "${text}", Kak! 🥲`);
			...
			case 'happymod': {
				...
				try {
					...
					if (results.length === 0) return newReply(`⚠️ SyadBotz gak nemu aplikasi di HappyMod dengan kata kunci "${text}", Kak! 🥲`);
			...
			if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				...
				try {
					...
					if (results.length === 0) return newReply(`⚠️ SyadBotz gak nemu ringtone dengan kata kunci "${text}", Kak! 🥲`);
									...
									buttons: [
										{
											...
											"buttonParamsJson": `{
												...
												"sections": [{
													...
													{
														"title": "🔍 Search Menu",
														"description": "Cari apa aja di sini, SyadBotz bantu nemuin kok~ 🔍",
